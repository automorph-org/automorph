...
//import io.json4s.{ACursor, Decoder, DecodingFailure, Encoder, HCursor, Json}
//
///** OpenRPC schema support for Json4s message codec plugin using JSON format. */
//private[automorph] object Json4sOpenRpc {
//
//  def encoder: Encoder[OpenRpc] = {
//    implicit val schemaEncoder: Encoder[Schema] = new Encoder[Schema] {
//
//      override def apply(schema: Schema): JValue = {
//        val fields = Seq(
//          schema.`type`.map(v => "type" -> JValue.fromString(v)),
//          schema.title.map(v => "title" -> JValue.fromString(v)),
//          schema.description.map(v => "description" -> JValue.fromString(v)),
//          schema.properties.map(v => "properties" -> JValue.obj(v.view.mapValues(apply).toSeq*)),
//          schema.required.map(v => "required" -> JValue.arr(v.map(JValue.fromString)*)),
//          schema.default.map(v => "default" -> JValue.fromString(v)),
//          schema.allOf.map(v => "allOf" -> JValue.arr(v.map(apply)*)),
//          schema.$ref.map(v => "$ref" -> JValue.fromString(v)),
//        ).flatten
//        JValue.obj(fields*)
//      }
//    }
